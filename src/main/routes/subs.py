...
from src.validators.subs_creator_validator import subs_creator_validator

from src.model.repositories.subscribers_repository import SubscribersRepository

from src.controllers.subscribers.subs_creator import SubscribersCreator
from src.controllers.subscribers.subs_manager import SubscriberManager


subs_route_bp = Blueprint('subs_route', __name__)

@subs_route_bp.route('/sub', methods=['POST'])
def create_new_sub():
    subs_creator_validator(request)
...
@subs_route_bp.route('/sub/link/<link>/event/<event_id>', methods=['GET'])
def subs_by_link(link, event_id):
    subs_repo = SubscribersRepository()
...
@subs_route_bp.route('/sub/ranking/event/<event_id>', methods=['GET'])
